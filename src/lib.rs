...
///
/// Redacts `doc` in place. Assumes `doc` is already decrypted if it was
/// encrypted (see `Document::is_encrypted` / `Document::decrypt`).
pub fn redact_document(doc: &mut Document, uncompressed: bool) {
    // -----------------------------------------------------------------------
    ...
    for id in &all_ids {
        ...
        let obj = redact_object(obj, uncompressed);
    ...
    for page_id in page_ids {
        ...
        for cid in content_ids {
            redact_content_stream(doc, cid, uncompressed);
        }
    }
    ...
    for id in form_xobject_ids {
        redact_content_stream(doc, id, uncompressed);
    }
}
